...
'''
    ...
    def handle_data(self, data):
        if self.meet_lineno_tag or self.islineNum and data.isdigit():
            try:
    ...
    def create_coverage_trs(self, files_coverage_info):
        ...
        for f, v in files_coverage_info.items():
            ...
            if os.path.exists(gcovrfilepath):
                ...
                source_pattern_str=r'class="lineno">(?:<pre>)?(\d+)(?:</pre>)?</td>';
                dest_pattern_str=r'class="lineno" style="background:red"><pre>NN  \1</pre></td>';
                ...
                for line in open(gcovrfilepath, 'r').readlines():
                    ...
                    if ps:
                        ...
                        lineno = int(ps.group(1))
                        if changed_line_list.count(lineno) == 1:
                            line = re.sub(source_pattern_str, dest_pattern_str, line)
